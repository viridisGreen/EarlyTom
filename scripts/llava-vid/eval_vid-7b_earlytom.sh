...
for TASKS in "${TASKS_LIST[@]}"; do
  if [ "$TASKS" == "mvbench" ]; then
    ...
    LAYERS_LIST=("10,21,23" "10,21,23" "10,21,23" "6,21,23")
  ...
  for i in "${!RETAIN_RATIOS[@]}"; do
    ...
    LOG_DIR="./logs/vid-7b-earlytom/$TASKS/$RETAIN_RATIO"
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/vid-7b-earlytom-${RETAIN_RATIO}-t${T}-k${INNER_k}-r${INNER_r}.log"
